...
def main():
    ...
    fh, fh_2 = plot_trajectory_decomposition(U_orth, W_decomp, targ_cond)
    fh.savefig(os.path.join(save_dir, 'Traj_decomp.pdf'))
    fh_2.savefig(os.path.join(save_dir, 'Traj_decomp_timecourses.pdf'))
...
def plot_trajectory_decomposition(U, W, cond):
    ...
    # Based on these plots, it seems that indices 5 or 6 are where the A->B and
    # B->A trajectories are most different.  Project neural activity into the
    # null space and run LDA on these points to find a plan of maximal
    # separability.

    # Get midpoint data
    mid_idx = 5
    W_null = W[:, 3:]  # The top 3 are potent (2) and condition-invariant
    U_null = U.apply(lambda u: W_null.T @ u)
    max_idx = [u.shape[1] - 1 for u in U_null]
    mid_idx_list = [i if i < mid_idx else mid_idx for i in max_idx]
    U_mid = [u[:, i] for u, i in zip(U_null, mid_idx_list)]
    X_mid = np.stack(U_mid).T  # dim x obs

    # Run LDA
    D_lda, J = neu.analysis.math.fisher_lda(X_mid, cond)

    # Project into LDA space
    U_lda = U_null.apply(lambda u: D_lda.T @ u)
    U_lda_avg, avg_cond = average_trajectories_standard(U_lda, cond)

    # Plot LDA projection(s)
    # Subplot 3: 2D plot of null dims. 1 and 2
    U_plot = U_lda_avg.apply(lambda u: u[0:2, :])
    curr_ax = axh[2][0]
    tmp.plot_traj(
        U_plot,
        pd.Series(avg_cond),
    ...
    curr_ax.set_xlabel('Null LDA dim. 1')
    curr_ax.set_ylabel('Null LDA dim. 2')
    curr_ax.set_title('Null space projection 1 (LDA)')

    # Subplot 4: 2D plot of null dims. 3 and 4
    U_plot = U_lda_avg.apply(lambda u: u[2:4, :])
    curr_ax = axh[2][1]
    tmp.plot_traj(
        U_plot,
        pd.Series(avg_cond),
    ...
    curr_ax.set_xlabel('Null LDA dim. 3')
    curr_ax.set_ylabel('Null LDA dim. 4')
    curr_ax.set_title('Null space projection 2 (LDA)')

    # Create new figure with just the time courses
    n_col = 5
    n_row = 2
    fh_2, axh = tmp.subplot_fixed(
        n_row, n_col, [300, 300],
        x_margin=[150, 150],
        y_margin=[150, 200]
    )
    ...
    # Plot time courses for each dimension
    plot_idx = itertools.product(range(n_row), range(n_col))
    for i, p in enumerate(plot_idx):
        curr_ax = axh[p[0]][p[1]]
        U_plot = U_decomp_avg.apply(lambda u: u[i, :])
        # Iterate over conditions
        for uni_cond, u in zip(avg_cond, U_plot):
            curr_ax.plot(u, color=col_map[uni_cond]['dark'])

        curr_ax.set_xlabel('Time (45ms bin)')
        curr_ax.set_ylabel('Null dim. {}'.format(i + 1))
        curr_ax.set_title(dim_names[i])
